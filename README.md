<div align="center">

# Hi, I'm João 👋

Software Engineering student at FEUP (University of Porto), exploring different areas of software development — from systems programming to robotics, game dev, and mobile apps.

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/jo%C3%A3o-dos-santos-w-960a9a314/?skipRedirect=true)
[![Email](https://img.shields.io/badge/-Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:joao.francisco.c.santos@email.com)

</div>

---

### 🎓 About me

- 💻 Currently studying **Bachelor in Informatics and Computing Engineering (LEIC)** at FEUP
- 🌍 Cambridge C1 English (in progress)
- 🔭 Still exploring my specialization — between **C++**, **robotics**, and **game/app development**
- 📚 Interested in systems and creative development (Godot, Flutter)
- ⚡ Outside of code: cycling, powerlifting, and karate

---

### 🚀 Featured projects

**[SVG to PNG Converter](https://github.com/JoaoFranciscoCSantos/svg_to_png)**
A C++ tool that parses SVG files (using TinyXML2) and renders them to PNG, with support for geometric shapes and transformations. Group project focused on XML parsing, geometry, and collaborative Git workflows.
`C++` `TinyXML2` `Parsing` `Geometry`

**Orion 2WD Robot**
A two-wheel-drive robotics project — exploring motor control, sensors, and embedded logic.
`Robotics` `C++/Embedded`

---

### 🛠️ Technologies & Tools

![C++](https://img.shields.io/badge/-C++-00599C?style=flat-square&logo=c%2B%2B&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/-GitHub-181717?style=flat-square&logo=github&logoColor=white)
![Linux](https://img.shields.io/badge/-Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![VSCode](https://img.shields.io/badge/-VS%20Code-007ACC?style=flat-square&logo=visual-studio-code&logoColor=white)
![Godot](https://img.shields.io/badge/-Godot-478CBF?style=flat-square&logo=godot-engine&logoColor=white)
![Flutter](https://img.shields.io/badge/-Flutter-02569B?style=flat-square&logo=flutter&logoColor=white)

---

### 📊 GitHub Stats

<div align="center">

![Stats](https://github-readme-stats.vercel.app/api?username=JoaoFranciscoCSantos&show_icons=true&theme=default&hide_border=true&count_private=true)
![Streak](https://github-readme-streak-stats.herokuapp.com/?user=JoaoFranciscoCSantos&theme=default&hide_border=true)

</div>

---

<div align="center">
<sub>Always learning, always building.</sub>
</div>
